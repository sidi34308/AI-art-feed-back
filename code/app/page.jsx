...
export default function Home() {
    ...
    async function fetchIdea() {
        setLoading(true);
        const prompt = "Generate one drawing idea, short in one sentence in arabic,make sure it is for a meaningful drawing or painting";
        try {
    ...
    async function lookforKeyWords(ideas) {
        const prompt = "from this idea text give me only list of key words that can be searched for reference images separated by , sign and no space between keywords, make sure the key words are in english do not add key words like, limit the key words to 8";
        try {
            const genAI = new GoogleGenerativeAI(API_KEY);
            const model = genAI.getGenerativeModel({ model: MODEL_NAME });

            const result = await model.generateContent([{ text: prompt }, { text: ideas }]);
            const text = result.response.text();
                ...
                <div className="w-full flex flex-col gap-6">
                    <div className="flex flex-col justify-center items-center p-8 bg-zinc-9200 rounded-[15px] border border-zinc-700">
                        <div className="flex flex-col justify-between items-center gap-6">
                            <h2 className="font-bold text-xl">توليد فكرة لوحة باستخدام الذكاء الاصطناعي</h2>
                            <button
